...
class CheckDerivedMemberAccess : public scoped_ref_ptr<SelfAssign> {
 public:
...
class ScopedRefPtrToSelf : public base::RefCounted<ScopedRefPtrToSelf> {
 public:
  ...
  scoped_ref_ptr<ScopedRefPtrToSelf> self_ptr_;
...
scoped_ref_ptr<Other> Overloaded(scoped_ref_ptr<Other> other) {
  return other;
}

scoped_ref_ptr<SelfAssign> Overloaded(scoped_ref_ptr<SelfAssign> self_assign) {
  return self_assign;
}
...
TEST(RefCountedUnitTest, TestSelfAssignment) {
  ...
  scoped_ref_ptr<SelfAssign> var(p);
  var = var;
  EXPECT_EQ(var.get(), p);
}
...
TEST(RefCountedUnitTest, ScopedRefPtrToSelfMoveAssignment) {
  ...
  check->self_ptr_ = scoped_ref_ptr<ScopedRefPtrToSelf>();
  EXPECT_TRUE(ScopedRefPtrToSelf::was_destroyed());
}

TEST(RefCountedUnitTest, ScopedRefPtrToOpaque) {
  scoped_ref_ptr<base::OpaqueRefCounted> p = base::MakeOpaqueRefCounted();
  base::TestOpaqueRefCounted(p);

  scoped_ref_ptr<base::OpaqueRefCounted> q;
  q = p;
  base::TestOpaqueRefCounted(p);
...
TEST(RefCountedUnitTest, BooleanTesting) {
  scoped_ref_ptr<SelfAssign> ptr_to_an_instance = new SelfAssign;
  ...
  scoped_ref_ptr<SelfAssign> null_ptr;
...
TEST(RefCountedUnitTest, Equality) {
  scoped_ref_ptr<SelfAssign> p1(new SelfAssign);
  scoped_ref_ptr<SelfAssign> p2(new SelfAssign);
...
TEST(RefCountedUnitTest, NullptrEquality) {
  scoped_ref_ptr<SelfAssign> ptr_to_an_instance(new SelfAssign);
  scoped_ref_ptr<SelfAssign> ptr_to_nullptr;
...
TEST(RefCountedUnitTest, ConvertibleEquality) {
  scoped_ref_ptr<Derived> p1(new Derived);
  scoped_ref_ptr<SelfAssign> p2;
...
TEST(RefCountedUnitTest, MoveAssignment2) {
  ...
  {
    ScopedRefPtrCountBase *raw = new ScopedRefPtrCountBase();
    scoped_ref_ptr<ScopedRefPtrCountBase> p1;
    EXPECT_EQ(1, ScopedRefPtrCountBase::constructor_count());
    EXPECT_EQ(0, ScopedRefPtrCountBase::destructor_count());

    {
      scoped_ref_ptr<ScopedRefPtrCountBase> p2(raw);
      EXPECT_EQ(1, ScopedRefPtrCountBase::constructor_count());
      EXPECT_EQ(0, ScopedRefPtrCountBase::destructor_count());
...
TEST(RefCountedUnitTest, MoveAssignmentSameInstance1) {
...

  {
    ...
    {
      scoped_ref_ptr<ScopedRefPtrCountBase> p2(p1);
      EXPECT_EQ(1, ScopedRefPtrCountBase::constructor_count());
      EXPECT_EQ(0, ScopedRefPtrCountBase::destructor_count());
...
TEST(RefCountedUnitTest, MoveAssignmentDifferentInstances) {
...

  {
    ...
    scoped_ref_ptr<ScopedRefPtrCountBase> p1(raw1);
    EXPECT_EQ(1, ScopedRefPtrCountBase::constructor_count());
    EXPECT_EQ(0, ScopedRefPtrCountBase::destructor_count());

    {
      ScopedRefPtrCountBase *raw2 = new ScopedRefPtrCountBase();
      scoped_ref_ptr<ScopedRefPtrCountBase> p2(raw2);
...
TEST(RefCountedUnitTest, MoveAssignmentDerived) {
...

  {
...

    {
      ...
      scoped_ref_ptr<ScopedRefPtrCountDerived> p2(raw2);
      EXPECT_EQ(2, ScopedRefPtrCountBase::constructor_count());
      EXPECT_EQ(0, ScopedRefPtrCountBase::destructor_count());
...
TEST(RefCountedUnitTest, MoveConstructor) {
...

  {
    ...
    {
      scoped_ref_ptr<ScopedRefPtrCountBase> p2(std::move(p1));
      EXPECT_EQ(1, ScopedRefPtrCountBase::constructor_count());
      EXPECT_EQ(0, ScopedRefPtrCountBase::destructor_count());
      EXPECT_EQ(nullptr, p1.get());
      EXPECT_EQ(raw, p2.get());
...
TEST(RefCountedUnitTest, MoveConstructorDerived) {
...

  {
    ...
    scoped_ref_ptr<ScopedRefPtrCountDerived> p1(raw1);
    EXPECT_EQ(1, ScopedRefPtrCountBase::constructor_count());
    EXPECT_EQ(0, ScopedRefPtrCountBase::destructor_count());
    EXPECT_EQ(1, ScopedRefPtrCountDerived::constructor_count());
    EXPECT_EQ(0, ScopedRefPtrCountDerived::destructor_count());
...
TEST(RefCountedUnitTest, TestOverloadResolutionCopy) {
  scoped_ref_ptr<Derived> derived(new Derived);
  scoped_ref_ptr<SelfAssign> expected(derived);
  EXPECT_EQ(expected, Overloaded(derived));

  scoped_ref_ptr<Other> other(new Other);
